...
protected:
	void insertaOrdenadoInterno(Link & raiz, T const& e) {
		if (raiz == nullptr) raiz = std::make_shared<TreeNode>(nullptr, e, nullptr);
		else if (raiz->elem > e) insertaOrdenadoInterno(raiz->left, e);
		else if (raiz->elem < e) insertaOrdenadoInterno(raiz->right, e);
	}
...
public:
   ...
   void insertaOrdenado(T e) {
	   insertaOrdenadoInterno(raiz, e);
   }
   ...
   std::vector<T> inorderValues(T k1, T k2) const {
	   std::vector<T> in;
	   inorderValores(raiz, in, k1, k2);
	   return in;
   }
...
protected:
   ...
   static void inorderValores(Link a, std::vector<T> & in, T k1, T k2) {
	   if (a != nullptr) {
		  if(a->elem > k1) inorderValores(a->left, in, k1, k2);
		  if (a->elem >= k1 && a->elem <= k2) in.push_back(a->elem);
		  if (a->elem < k2) inorderValores(a->right, in, k1, k2);
	   }
   }
